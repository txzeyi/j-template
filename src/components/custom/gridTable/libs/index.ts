import { ICellData } from "../index2.vue"
...
export default class GridTable {
  ...
  data: ICellData[] = []
  on = {}
  render: null | Function | Object = null
  style = {}
  validate: null | Function | Object = null
  resize = false
  quadrantType = 0
  showSummary = true
  canNumberInput = true
  ...
  cellWidth = 0
  cellHeight = 0
  quadrantData: ICellData[] = []
  dataMap = {}

  constructor(options: any, wxScope?: any) {
    this.el = options.el
    ...
    getCanvas(this, this.scope).then((res) => {
      ...
      this.canvas && (this.canvas.ctx = this.ctx = this.canvas.getContext("2d"))
      this.data = opt.data
      this.on = opt.on
